...
def main():
    ...
    while True:
        ...
        end_list=driver.find_elements_by_class_name("cassetteRecruit__endDate")
        # 1ページ分繰り返し
        print("{},{},{},{}".format(len(copy_list),len(status_list),len(name_list),len(end_list)))
        for name,copy,status,endday in zip(name_list,copy_list,status_list, end_list):
            print(name.text)
            print(copy.text)
            print(status.text)
            print(endday.text)
